...
export class ModelRouterService {
  ...
  public async route(
    ...
    console.log('ModelRouterService: We are entering the route function.');
    ...
    //   console.log('ModelRouterService: Less Than 5 Parts.');
    //   return {
    ...
    if (turnType === 'tool_response' || turnType === 'next_speaker_request') {
      console.log('ModelRouterService: Function | Speaker request');
      return {
        model: this.config.getModel(),
    ...
    if (this.config.isInFallbackMode()) {
      console.log('ModelRouterService: fallback checker');
      return {
    ...
    if (context.forcedModel) {
      console.log('ModelRouterService: Forced Model Checker');
      return {
