...
export class ClassifierStrategy implements RoutingStrategy {
  async route(
    ...
    console.log(
      `ClassifierStrategy: Turn Type: ${context.turnContext.turnType}`,
    );
    console.log(
      `ClassifierStrategy: Routing request: ${JSON.stringify(context.request)}`,
    );
    try {
      ...
      console.log(
        `ClassifierStrategy: Model chosen: ${routerResponse.model_choice}, Reasoning: ${routerResponse.reasoning}`,
      );
